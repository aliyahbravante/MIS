...
import '../student/studentpaymentrecords.css';

const PStudentPaymentRecords = () => {
    ...
    try {
      ...
      if (data.success) {
        if (data.status === "payment_processed" || data.status === "already_processed") {
          ...
          console.log('Payment verified as PAID by Paymongo');
          ...
          fetchPaymentRecords(); // Refresh records
        ...
        setError(`Payment verification error: ${data.message}`);
        
        // Add this line to fetch records even on API error
        fetchPaymentRecords();
      }
    } catch (error) {
      console.error("Payment verification error:", error);
      
      // Add this line to fetch records even on exception
      fetchPaymentRecords();
    } finally {
  ...
  const generatePDF = () => {
    ...
    sortedPaymentRecords.forEach((record) => {
      tableRows.push([
        ...
        record.payment,
        record.date,
        parseFloat(record.total_fee).toFixed(2),
        parseFloat(record.amount_paid).toFixed(2),
        parseFloat(record.balance).toFixed(2), // Use the balance from the database
        record.remarks,
    ...
    if (sortedPaymentRecords.length > 0) {
      ...
      doc.setFontSize(12);
      doc.text(`Current Balance: ${parseFloat(latestRecord.balance).toFixed(0)}`, 14, doc.lastAutoTable.finalY + 15);
    }
  ...
  const handleExportExcel = () => {
    ...
    const exportData = sortedPaymentRecords.map((record) => {
      return {
        ...
        "Date": record.date,
        "Total Fee": parseFloat(record.total_fee).toFixed(2),
        "Amount Paid": parseFloat(record.amount_paid).toFixed(2),
        "Balance": parseFloat(record.balance).toFixed(2), // Use database balance
            ...
            <tbody>
              {sortedPaymentRecords.map((record, index) => (
                <tr key={index}>
                  ...
                  <td>{record.date}</td>
                  <td className="amount-column" style={{ textAlign: 'right' }}>{formatCurrency(record.total_fee)}</td>
                  <td className="amount-column" style={{ textAlign: 'right' }}>{formatCurrency(record.amount_paid)}</td>
                  <td className="amount-column" style={{ textAlign: 'right' }}>{formatCurrency(record.balance)}</td>
      ...
      {hasRecords && (
        <div className="balance-summary-spay">
          ...
          <input
            ...
            value={formatCurrency(latestPayment.balance)}
            readOnly
        ...
        <div className="modal-confirmation-spay payment-modal">
          <div className="modal-content-spay payment-content">
            ...
            <div className="payment-info">
              ...
              <p><strong>Current Balance:</strong> {formatCurrency(latestPayment.balance)}</p>
            </div>
...
export default PStudentPaymentRecords;